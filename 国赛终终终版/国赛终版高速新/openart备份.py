...
YELLOW_THRESHOLD = [(59, 100, -31, 4, 57, 127)]
...
_FRAME2_TYPE = 0x07
_tx_frame2 = bytearray(12 + MAX_SEND_OBJECTS * 4)
...
def uart_get():
    ...
    #     bit3=count_mode，bit4~6=total_remaining，bit7保留并固定为0
    ...
    if uart.any():
        chunk = uart.read(min(uart.any(), 120))
        if chunk:
            if len(uart_buf) + len(chunk) > 400:
                uart_buf = bytearray()
            uart_buf += chunk
...
# ── 模型加载 ──────────────────────────────────────────────────
# 1) 先加载模型、后启动 Timer：避免大文件 SD 读取期间被 500Hz 中断
#    （UART 读 + uart_buf 堆分配）干扰，这是间歇性读失败的嫌疑源之一。
...
while True:
    ...
    # YOLO 前提前发一次：当前帧位姿 + 上一帧目标，减少 RT1021 等待时间。
    uart_send_objects(
        vis_yaw_calc if vis_yaw_calc is not None else 999.0,
   ...
   # global _sel_locked, _sel_lock_rel_x, _sel_lock_rel_y, _sel_lock_type, _sel_miss_frames
    ...
    # 视觉 yaw 不再经过“接近90度/接近IMU/累计次数”门控；本帧能解算就直接发送。
