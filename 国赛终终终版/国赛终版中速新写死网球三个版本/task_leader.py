...
_SEARCH_FIRST_DIAG_END_X = 200.0
_SEARCH_FIRST_DIAG_END_Y = 55.0
_SEARCH_FIRST_DIAG_SPEED = 150.0

# 开局首次斜移到位后的网球直搜线。主车先原地锁到 0°，
# 再沿车头正前方以速度 80 持续直行，未锁定网球时不停车、不超时。
_SEARCH_FIRST_STRAIGHT_YAW = 0.0
_SEARCH_FIRST_STRAIGHT_SPEED = 80.0
...
_SEARCH_LINE_X_AFTER_PUSH_270 = 80.0
...
_RECOVER_FIX_SPEED = const(140)

_RECOVER_FIX_TIMEOUT_MS = const(800)
...
_RELOC_DRIVE_TIMEOUT_MS = const(4000)
...
_SEARCH_FIRST_STRAIGHT = const(12)
...
# 首次固定斜移结束后，主车必须先原地转到网球直搜航向，再开始直行。
# 航向需连续落在误差带内一小段时间，避免刚穿过目标角就立即开始移动。
...
def _reset_search():
    ...
    # 首次搜索固定使用网球直搜航向；以后采用最近一次推出方向的反方向。
    ...
    if first_search:
        raw_yaw = _SEARCH_FIRST_STRAIGHT_YAW
...
def _leader_search_target_seen():
    ...
    if not 1 <= obj_id < len(_obj_remain) or _obj_remain[obj_id] <= 0:
        ...
        _search_seen_cnt = 0
        return False
    # 首个目标硬限制：配置类别未完成前，即使旧视觉帧传来了其他物体，
    # 任务层也不会锁定。这与 base.uart_send_pose() 的摄像头类别掩码构成双重保险。
    first_obj_id = int(getattr(config, "_FIRST_TARGET_OBJ_ID", 0))
    if (1 <= first_obj_id < len(_obj_remain)
            and _obj_remain[first_obj_id] > 0
            and obj_id != first_obj_id):
        _search_seen_id = 0
        _search_seen_cnt = 0
        return False
...
def _update_search(now_ms):
    ...
    # 首次斜移和原地转向不锁物，保证主车必须先完成预设路线入口。
    # 进入 0°/80 网球直搜后才允许锁定；后续普通横扫和换行仍保持原逻辑。
    target_state = (
        _mode_sub == _SEARCH_FIRST_STRAIGHT
    ...
    if _mode_sub == _SEARCH_FIRST_TURN_YAW:
        ...
        if abs(_angle_diff(base._car.current_angle, _search_yaw)) <= _SEARCH_YAW_EPS_DEG:
            if _search_sub_t0_ms == 0:
                ...
                # 航向稳定后进入专用网球直搜，从车继续动态跟随修正。
                _mode_sub = _SEARCH_FIRST_STRAIGHT
                _self_sub = _mode_sub
                _search_sub_t0_ms = now_ms
                base.request_world(0.0, _SEARCH_FIRST_STRAIGHT_SPEED, _SEARCH_FIRST_STRAIGHT_YAW)
        else:
            _search_sub_t0_ms = 0
        return

    if _mode_sub == _SEARCH_FIRST_STRAIGHT:
        # 不读取搜索边界、黄线、障碍换行和 SEARCH 超时；没锁到网球就一直前进。
        _self_ready = 0
        base.request_world(0.0, _SEARCH_FIRST_STRAIGHT_SPEED, _SEARCH_FIRST_STRAIGHT_YAW)
        return
