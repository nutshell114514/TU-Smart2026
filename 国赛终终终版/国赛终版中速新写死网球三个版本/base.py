...
def uart_send_pose():
    ...
    first_pending = (1 <= first_obj_id < len(task._obj_remain)
                     and task._obj_remain[first_obj_id] > 0)
    for i in range(1, min(_OBJ_ID_MAX, len(task._obj_remain) - 1) + 1):
        # 第一目标未完成前，只把它开放给 OpenART；完成后自动恢复所有剩余类别。
        if task._obj_remain[i] > 0 and (not first_pending or i == first_obj_id):
            remain_mask |= 1 << i - 1
