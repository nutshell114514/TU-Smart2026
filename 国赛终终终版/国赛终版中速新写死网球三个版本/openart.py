...
,
    ...
    2: {#有黄色胶带的，在这里定义2是从车
        ...
        "car_screen_x": 155,
        "car_screen_y": 239,
        "qvga_matrix":[-2.4078946343265883, -1.5479322673010578, 376.8355102887487, -3.4715811589885253e-17, 4.621683938162651, -549.9803889138338, -1.2167828878797342e-17, -0.14285714285714224, 0.9999999999999999]
    },
}
...
# 首目标硬限制与主控 config._FIRST_TARGET_OBJ_ID=1 保持一致。
# OpenART 在收到第一帧主控指令前也只开放网球，避免上电窗口误锁其他物体。
FIRST_TARGET_OBJ_ID    = 1
current_obj_remain     = [0, 1, 0, 0, 0, 0]
...
# 红沙包/红砖同色误识别的第二道过滤：世界坐标校验。原本放在下位机
# （用本车里程计换算 world_x 再判断），改到上位机是因为这里的 current_imu_x/y
# 本来就是每帧从下位机同步过来的同一份位姿，判断依据完全一致，但可以在
# 检测结果送出前就滤掉，不用再占用下位机的解析逻辑和 CPU 时间。
# 红砖固定摆在低 X 场边，待搬红沙包只会出现在搜索区内，因此"不在推红沙包、
# 又出现在低 X 区域"的红沙包判为红砖误检，直接丢弃。
RED_SANDBAG_X_FILTER_ENABLE = 1
RED_SANDBAG_X_FILTER_CM = 70.0
...
def uart_get():
    ...
    global _leader_sel_candidate_type, _leader_sel_candidate_rel_x
    global _leader_sel_candidate_rel_y, _leader_sel_candidate_count, _leader_sel_observe_t0
    ...
    if last_valid >= 0:
        ...
        # 已知类别模式下，只要网球仍在剩余掩码中，OpenART 就强制只开放网球。
        # 网球成功推出后主控会清掉该位，此时自动放开沙包和熊。
        if (count_mode == 0
                and current_obj_remain[FIRST_TARGET_OBJ_ID] > 0):
            for j in range(1, 6):
                current_obj_remain[j] = 1 if j == FIRST_TARGET_OBJ_ID else 0

        # 掩码收紧时立即清掉已被禁用类别的旧锁和候选，不等丢失容忍帧数超时。
        locked_enabled = (
            current_obj_total_remaining > 0 if count_mode == 1
            else (1 <= _sel_lock_type <= 5 and current_obj_remain[_sel_lock_type] > 0)
        )
        if _sel_locked and not locked_enabled:
            _sel_locked = False
            _sel_lock_rel_x = 999.0
            _sel_lock_rel_y = 999.0
            _sel_lock_type = 0
        ...
        candidate_enabled = (
            current_obj_total_remaining > 0 if count_mode == 1
            else (1 <= _leader_sel_candidate_type <= 5
                  and current_obj_remain[_leader_sel_candidate_type] > 0)
        )
        if _leader_sel_candidate_type > 0 and not candidate_enabled:
            _leader_sel_candidate_type = 0
            _leader_sel_candidate_rel_x = 999.0
            _leader_sel_candidate_rel_y = 999.0
            _leader_sel_candidate_count = 0
...
def uart_send_objects(vis_yaw, rod_x, rod_y, line_cx, obj_list):
    ...
    for i in range(SEND_OBJECT_SLOTS):
        ...
        if (i == 0 and current_target_sel_id > 0 and len(obj_list)
                ...
                and obj_enabled(obj_list[0]["class_id"])):
            best_idx = 0
            ...
            for j in range(len(obj_list)):
                ...
                if obj["class_id"] <= 0 or obj["class_id"] > 5:
                    continue
                if not obj_enabled(obj["class_id"]):
                    continue
...
while True:
    ...
    for det in detections:
        ...
        world_x, world_y, world_valid = compute_obj_world_from_rel(rel_x, rel_y)
        if (class_id == 3
                and RED_SANDBAG_X_FILTER_ENABLE
                and world_valid
                and world_x < RED_SANDBAG_X_FILTER_CM
                and current_target_sel_id != 3):
            # 世界坐标落在低 X 场边、且当前没有在推红沙包：判为红砖误检，丢弃。
            # 正在推红沙包时不过滤，避免把正在处理的真沙包也滤掉。
            continue
