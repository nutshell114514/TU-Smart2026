...
两台全向麦轮智能车通过无线组队，在 310 cm × 230 cm 的场地中自主搜索目标物体、编队接近、**双车 V 字夹持同步推送**，把网球 / 沙包 / 玩偶推出指定边界，全程自主避障，任务结束后各自重定位返回起点并完成绕圈展示。

整套系统跑在 MicroPython 上：每台车一块 RT1021 主控 + 一个 OpenART 视觉模块，两车之间靠 230400 baud 无线串口做半双工状态同步，没有任何上位机参与决策。
...
单台全向车推动网球、沙包这类物体时，接触点只有一个，物体极易从推杆侧滑脱出。本系统的核心思路是**用两台车形成 V 字夹持阵型**：主车从目标的一侧靠近，从车绕到另一侧，两车推杆在物体前方合围成 V 形，然后以同步的速度和航向共同推送。这样物体被两个方向的力约束，横向自由度被消除，可以稳定地被推出场地边界。
